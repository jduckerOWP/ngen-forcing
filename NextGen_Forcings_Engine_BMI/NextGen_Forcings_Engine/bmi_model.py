...
# BMI grid function class 
from .bmi_grid import Grid, GridType

# Forcing Engine core modules
from .core import (
    config,
    err_handler,
    forcingInputMod,
...
# Forcing Engine workflow class
from .model import NextGenForcingEngineModel


# Python type hinting modules
...
def _preprocess(v, separator, ignorecase):
    """Preprocess version string into comparable list elements."""
...
class NextGen_Forcing_Engine_BMI_model(Bmi):
    """Defines the BMI (Basic Model Interface) for the NextGen Forcings Engine model.
    ...
    def __init__(self):
        ...
        """
        super(NextGen_Forcing_Engine_BMI_model, self).__init__()
        self._values = {}
    ...
    # ----------------------------------------------
    _att_map = {
        "model_name": "NextGen Forcings Engine BMI Python",
        "version": "1.0",
    ...
    def initialize(self, config_file: str, output_path: str | None = None) -> None:
        ...
        self._model = NextGenForcingEngineModel()
    ...
    def initialize_with_params(
        ...
        """Initialize the NextGen Forcings Engine model with additional job metadata parameters.

        This function **must be called by the user** to fully initialize the NextGen Forcings Engine model,
    ...
    def finalize(self):
        ...
        if self._mpi_meta.rank == 0:
            for filename in os.listdir(self._job_meta.scratch_dir):
                ...
                # Keep the NextGen Forcing Engine netcdf output file as well ESMF weight files for reuse in following runs
    ...
    def _parse_config(self, cfg: dict) -> dict:
        ...
        for key, val in cfg.items():
            ...
            # Configure NextGen input configurations to what the ConfigClass expects
